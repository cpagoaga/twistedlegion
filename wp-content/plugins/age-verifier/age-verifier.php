...
Plugin URI: https://www.webcraftplugins.com/
Version: 1.4.1
...
if (!class_exists('AgeVerifier')) {
	class AgeVerifier {
		...
		function admin_includes() {
			...
			wp_enqueue_style('bootstrap-wpfix-css', plugins_url('/css/libs/bootstrap.localized.css', __FILE__), false, '1.4.1', false);
			wp_enqueue_style('bootstrap-css', plugins_url('/css/libs/bootstrap.wpfix.css', __FILE__), false, '1.4.1', false);
			wp_enqueue_script('boostrap-js', plugins_url('/js/libs/bootstrap.min.js', __FILE__), false, '1.4.1', true);

			wp_enqueue_style('chosen-css', plugins_url('/css/libs/chosen.min.css', __FILE__), false, '1.4.1', false);
			wp_enqueue_script('chosen-js', plugins_url('/js/libs/chosen.jquery.min.js', __FILE__), false, '1.4.1', true);

			wp_enqueue_style('colorpicker-css', plugins_url('/css/libs/colorpicker.css', __FILE__), false, '1.4.1', false);
			wp_enqueue_script('colorpicker-js', plugins_url('/js/libs/colorpicker.js', __FILE__), false, '1.4.1', true);

			wp_enqueue_script('age-verifier-module-filters-js', plugins_url('/js/age-verifier-module-filters.js', __FILE__), false, '1.4.1', true);

			wp_enqueue_style('age-verifier-editor-css', plugins_url('/css/age-verifier-editor.css', __FILE__), false, '1.4.1', false);
			wp_enqueue_script('age-verifier-editor-js', plugins_url('/js/age-verifier-editor.js', __FILE__), false, '1.4.1', true);
			wp_enqueue_script('age-verifier-content-js', plugins_url('/js/age-verifier-content.js', __FILE__), false, '1.4.1', true);
			wp_enqueue_script('age-verifier-jquery-generator-js', plugins_url('/js/age-verifier-jquery-generator.js', __FILE__), false, '1.4.1', true);

			wp_enqueue_style('age-verifier-admin-css', plugins_url('/css/admin.css', __FILE__), false, '1.4.1', false);
			wp_enqueue_script('age-verifier-admin-js', plugins_url('/js/admin.js', __FILE__), false, '1.4.1', true);

			wp_enqueue_script('age-verifier-wp-ajax-js', plugins_url('/js/age-verifier-wp-ajax.js', __FILE__), false, '1.4.1', true);

			wp_enqueue_style('age-verifier-css', plugins_url('/css/age-verifier.css', __FILE__), false, '1.4.1', false);
			wp_enqueue_script('age-verifier-js', plugins_url('/js/age-verifier.js', __FILE__), false, '1.4.1', true);
		}
		function client_includes() {
			if ($this->should_include_plugin()) {
				$options = $this->get_admin_options();

				if ($options['settings']['hide_site'] == 1) {
					echo '<style>body, html { opacity: 0; }</style>';
				}
		...
		function print_options_page() {
			...
			<div id="placeholder_image_url" class="hidden">
				<?php echo plugins_url('img/admin/placeholder_image.png', __FILE__); ?>
			</div>

			<div id="main-footer" class="col-md-12">
				<ul class="admin-panel-section">
					<li>
						<a href="https://webcraftplugins.com/support" id="footer-support" target="blank">
							<img src="<?php echo plugins_url('img/admin/support.png', __FILE__); ?>">
						</a>
					</li>
					<li>
						<a href="https://webcraftplugins.com/" id="footer-logo" target="blank">
			...
			if ($this->should_include_plugin()) {
				...
				;(function ( $, window, document, undefined ) {
					$(document).ready(function() {
						$.age_verifier_id = '<?php echo $settings['cookie_id']; ?>';
						$.age_verifier_ajax_url = '<?php echo admin_url('admin-ajax.php'); ?>';
						$.AgeVerifier(<?php echo json_encode($settings['settings']); ?>);
					});
		...
		function set_cookies() {
			if (is_admin()) return;
			if (!$this->should_include_plugin()) return;
		...
		function should_include_plugin() {
			...
			if (is_object($options['settings'])) {
				$options['settings'] = (array) $options['settings'];
			}

			if (intval($options['settings']['enabled']) == 1 && $this->filter()) {
				if ((isset($_COOKIE[$cookie_name]) && $_COOKIE[$cookie_name] == 'unverified') || !isset($_COOKIE[$cookie_name])) {
					return true;
				}
			}
		...
		function filter() {
			...
			if (!isset($options['settings']['filters'])) return true;
		...
		function user_verified() {
			...
			setcookie($cookie_name, 'verified', time()+60*60*24*$cookie_expiration, '/', "", false, true);
			die();
		}
	                ...
	                <form class="form-horizontal">
	                    ...
	                    <div class="form-group">
	                        ...
	                        <div class="col-md-9">
								...
								<div class="color-box image-box">
	                                <div class="image-box-inner"></div>
	                            </div>
	                            <div class="clear"></div>

								<input class="input-hidden" type="text" id="input-background-image-url">
	                        </div>
	                    </div>
	                    ...
	                    <div class="form-group">
	                        ...
	                        <div class="col-md-9">
	                            <div id="color-box-text-color" data-color="#ffffff">
	                                <span class="glyphicon glyphicon-edit"></span>
	                            </div>
	                        </div>
	                    </div>
						...
						<!-- Hide Site -->
						<div class="form-group">
							<label for="checkbox-hide-site" class="col-md-3 control-label">Hide Website <br>Behind the Age Verification Screen:</label>
							<div class="col-md-9">
								<div class="checkbox">
									<label>
										<input id="checkbox-hide-site" type="checkbox" style="width: 18px;">
									</label>
								</div>
							</div>
						</div>
		...
		function delete_cookies() {
			...
			setcookie($cookie_name, 'unverified', 0, '/', "", false, true);
...
add_action('wp_ajax_user_verified', array($instance, 'user_verified'));
